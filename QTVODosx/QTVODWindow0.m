...
{ QTAMovieView *qv = [self getView];
  ...
  BOOL vis = ![qv isControllerVisible];
	...
	NSLog( @"%@ current ACSCcount = %lu", [theURL path], ACSCcount );
}
...
{ QTMovie *m = [QTMovie movieWithURL:absoluteURL error:outError];
			...
			if( flashtmp.length ){
			  const char *from, *to;
				...
				flv = [fileName stringByAppendingString:
						[[NSString stringWithCString:mktemp(".QTAmtr") encoding:NSUTF8StringEncoding] stringByAppendingString:@".flv"] ];
	...
	{ static char active = 0;
		if( !active ){
			active = 1;
			m2 = [QTVODWindow alloc];
			if( m2 ){
				m2->isProgrammatic = TRUE;
				[NSBundle loadNibNamed:@"QTVODWindow" owner:m2];
				absoluteURL = [NSURL fileURLWithPath:@"/Volumes/Debian/Users/bertin/work/src/MacOSX/QTilities/QTils/Sample.mov"];
				[m2 initWithContentsOfURL:absoluteURL ofType:typeName error:outError];
			}
			active = 0;
		}
	}
...
{ QTVODWindow *mydoc= (QTVODWindow*) refCon;
  ...
  extern Boolean QTMovieWindow_MCActionHandler( MovieController mc, short action, void *params, long refCon);
	...
	if( action == mcActionControllerSizeChanged ){
		if( mydoc->qtmwH && !(*(mydoc->qtmwH))->theView ){
			(*(mydoc->qtmwH))->theView = [[mydoc getView] window];
			if( (*(mydoc->qtmwH))->theView ){
				register_QTMovieWindowH( (mydoc->qtmwH), (*(mydoc->qtmwH))->theView );
				[mydoc showWindows];
			}
		}
#if 0
		if( mydoc != m2 && m2 ){
		  Cartesian pos, size;
			QTMovieWindowGetGeometry( m2->qtmwH, &pos, &size, 1 );
			if( pos.horizontal != 10 || pos.vertical != 10 ){
				pos.horizontal = pos.vertical = 10;
				// set position and scale of the content region:
				QTMovieWindowSetGeometry( m2->qtmwH, &pos, NULL, 0.5, 0 );
				// get the resultant details for the envelope:
				QTMovieWindowGetGeometry( m2->qtmwH, &pos, &size, 1 );
				pos.horizontal += size.horizontal;
				pos.vertical += size.vertical;
				QTMovieWindowSetGeometry( mydoc->qtmwH, &pos, NULL, 1.0, 1 );
			}
		}
#endif
	}
	...
	switch( action ){
		case mcActionControllerSizeChanged:{
			mydoc->ACSCcount += 1;
#ifndef USE_QTILS
		  Rect bounds, theMovieRect;
		  char msg[512];
			MCGetControllerBoundsRect(mc, &bounds);
			GetMovieBox( [[mydoc getMovie] quickTimeMovie], &theMovieRect );
			...
			[mydoc setMovieSize:NSMakeSize(theMovieRect.right, theMovieRect.bottom) caller:"mcActionControllerSizeChanged" ];
			snprintf( msg, sizeof(msg), "ControllerSizeChanged: %hdx%hd+%hd+%hd movieRect=%hdx%hd+%hd+%hd",
				...
				theMovieRect.right, theMovieRect.bottom, theMovieRect.left, theMovieRect.top
			);
			mcFeedBack( mydoc, msg );
#endif
			break;
		}
...
#endif
#ifdef USE_QTILS
	return QTMovieWindow_MCActionHandler( mc, action, params, (long) mydoc->qtmwH );
#else
	return FALSE;
#endif
}
